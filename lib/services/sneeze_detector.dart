...
class SneezeDetector {
  ...
  void addFrame(Face face) {
    if (_buffer.isNotEmpty) {
      final timeDiff = DateTime.now()
          .difference(_buffer.last.timestamp)
          .inMilliseconds;
      if (timeDiff > 500) {
        clearBuffer();
      }
  ...
  SneezeReport? evaluateSneeze() {
    ...
    for (var frame in _buffer) {
      ...
      final pitchDelta =
          (baseFace.headEulerAngleX ?? 0.0) - (face.headEulerAngleX ?? 0.0);
    ...
    if (maxPitchDelta.abs() > requiredPitchDelta ||
        maxMovementDist > requiredMovement) {
